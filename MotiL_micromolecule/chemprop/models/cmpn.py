...
class CMPNEncoder(nn.Module):
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        self.mlp = nn.Linear(self.hidden_size*3, self.hidden_size, bias=self.bias)
        

    def forward(self, step, mol_graph, task, depth, dropout, features_batch=None) -> torch.FloatTensor:
        ...
        if task == 'contrast_mol':
            f_atoms, f_bonds, a2b, b2a, b2revb, a_scope, _, _ = mol_graph.get_components()
            ...
            agg_message = index_select_ND(message_bond, a2b)
            agg_message = agg_message.sum(dim=1) * agg_message.max(dim=1)[0]
            agg_message = self.mlp(torch.cat([agg_message, message_atom, input_atom], 1))
            agg_message = self.gru(agg_message, a_scope)
            atom_hiddens = self.act_func(self.W_o(agg_message))  # num_atoms x hidden
            # atom_hiddens = self.act_func(agg_message)
            ...
            f_atoms, f_bonds, a2b, b2a, b2revb, _, _, all_fgs = mol_graph.get_components()
            ...
            fg_vecs = []
            for key in all_fgs.keys(): # loop for one type of fg
                fg_emb = []
                for fg in all_fgs[key]: # loop for one fg
                    atom_index = fg
                    a2b_fg = a2b[[0]+atom_index].clone()
                    bond_index = a2b_fg.reshape(-1)
                    b2a_fg = b2a[bond_index]
                    b2revb_fg = b2revb[bond_index]
                    atom_index = [0]+atom_index

                    input_atom = self.W_i_atom(f_atoms)  # num_atoms x hidden_size
                    input_atom = self.act_func(input_atom)
                    message_atom = input_atom.clone()
                    input_bond = self.W_i_bond(f_bonds)  # num_bonds x hidden_size
                    message_bond = self.act_func(input_bond).clone()
                    input_bond = self.act_func(input_bond).clone()
                    # Message passing
                    for depth in range(depth - 1):
                        agg_message = index_select_ND(message_bond, a2b_fg)
                        agg_message = agg_message.sum(dim=1) * agg_message.max(dim=1)[0]
                        # message_atom = message_atom + agg_message
                        message_atom[atom_index] = message_atom[atom_index] + agg_message
                        # directed graph
                        rev_message = message_bond[b2revb_fg]  # num_bonds x hidden
                        message_bond[b2revb_fg] = message_atom[b2a_fg] - rev_message  # num_bonds x hidden
                        message_bond = self._modules[f'W_h_{depth}'](message_bond)
                        message_bond = self.dropout_layer(self.act_func(input_bond + message_bond))
                    agg_message = index_select_ND(message_bond, a2b_fg)
                    agg_message = agg_message.sum(dim=1) * agg_message.max(dim=1)[0]
                    agg_message = self.mlp(torch.cat([agg_message, message_atom[atom_index], input_atom[atom_index]], 1))
                    # cur_scope = [(1, len(atom_index)-1)]
                    # agg_message = self.gru(agg_message, cur_scope)
                    # atom_hiddens = self.act_func(self.W_o(agg_message))  # num_atoms x hidden
                    atom_hiddens = self.act_func(agg_message)
                    atom_hiddens = self.dropout_layer(atom_hiddens)  # num_atoms x hidden
                    # Readout
                    cur_hiddens = atom_hiddens.mean(0)
                    fg_emb.append(cur_hiddens)
                fg_emb_tensor = torch.stack(fg_emb, dim=0)
                fg_avg_emb = fg_emb_tensor.mean(0)
                fg_vecs.append(fg_avg_emb)
            fg_vecs_final = torch.stack(fg_vecs, dim=0)

            return fg_vecs_final  # B x H
...
class CMPN(nn.Module):
    ...
    def forward(self, step, task, depth, dropout, batch,
                features_batch: List[np.ndarray] = None) -> torch.FloatTensor:
        if not self.graph_input:  # if features only, batch won't even be used
            batch = mol2graph(batch, self.args)

        output = self.encoder.forward(step, batch, task, depth, dropout, features_batch)
        return output
