...
def match_fg_all(mol):
    all_fgs = {}
    for sm in smart:
        fgs_tuple = mol.GetSubstructMatches(sm)
        num = len(fgs_tuple)
        if num > 0 and len(fgs_tuple[0]) > 1:
            all_match = []
            all_fgs[smart2name[sm]] = [list(fgs_tuple[0])]
    return all_fgs
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs, args: Namespace):
        ...
        self.all_fgs = {}   # fg's atom indexing
        ...
        for mol_graph in mol_graphs:
            ...
            # for key in mol_graph.all_fgs:
            #     mol_graph.all_fgs[key] = [[element + self.n_atoms for element in fgs] for fgs in mol_graph.all_fgs[key]]
            #     if key in self.all_fgs:
            #         self.all_fgs[key].extend(mol_graph.all_fgs[key])    # may be time-consuming and may require to optimize it!!!
            #     else:
            #         self.all_fgs[key] = mol_graph.all_fgs[key]

            for key in mol_graph.all_fgs:
                if len(self.all_fgs) > 9:
                    break
                mol_graph.all_fgs[key] = [[element + self.n_atoms for element in fgs] for fgs in mol_graph.all_fgs[key]]
                if key not in self.all_fgs:
                    self.all_fgs[key] = mol_graph.all_fgs[key]
    ...
    def get_components(self) -> Tuple[torch.FloatTensor, torch.FloatTensor,
        ...
        """
        return self.f_atoms, self.f_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope, self.atom_num, self.all_fgs
